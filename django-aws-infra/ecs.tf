locals {
    prod_rds_hostname = aws_db_instance.prod.address
}
...
resource "aws_ecs_task_definition" "prod_backend_web" {
  ...
  container_definitions = templatefile(
    "templates/backend_container.json.tpl",
    {
      ...
      rds_uri = "postgresql://${var.prod_rds_username}:${var.prod_rds_password}@${local.prod_rds_hostname}:5432/${var.prod_rds_db_name}"
    },
  )
